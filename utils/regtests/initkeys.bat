...
%TPM_EXE_PATH%create -hp 80000000 -ecc !CURVE[%%i]! -halg !SHALG[%%i]! -sir -opr signecc!CURVE[%%i]!nfpriv.bin -opu signecc!CURVE[%%i]!nfpub.bin -opem signecc!CURVE[%%i]!nfpub.pem -pwdp sto -pwdk sig > run.out
    IF !ERRORLEVEL! NEQ 0 (
      exit /B 1
    )
