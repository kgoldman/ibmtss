...
for %%B in (2048 3072) do (
    ...
    for %%H in (%ITERATE_ALGS%) do (
	    ...
	    %TPM_EXE_PATH%sign -hk 80000001 -halg %%H -salg rsa -scheme %%S -if policies/aaa -os sig.bin -pwdk sig -ipu signrsa%%Bpub.bin -v > run.out
	    IF !ERRORLEVEL! NEQ 0 (
	       exit /B 1
	    )

	    echo "Verify the signature signature using the TPM - %%H"
	    %TPM_EXE_PATH%verifysignature -hk 80000001 -halg %%H -rsa -if policies/aaa -is sig.bin > run.out
	    IF !ERRORLEVEL! NEQ 0 (
	       exit /B 1
	    )
...
openssl ecparam -name prime256v1 -genkey -noout -out tmpkeypaireccnistp256.pem > run.out
openssl ecparam -name secp384r1  -genkey -noout -out tmpkeypaireccnistp384.pem > run.out

echo "Convert key pair to plaintext DER format"

openssl ec -inform pem -outform der -in tmpkeypaireccnistp256.pem -out tmpkeypaireccnistp256.der -passin pass:rrrr > run.out
openssl ec -inform pem -outform der -in tmpkeypaireccnistp384.pem -out tmpkeypaireccnistp384.der -passin pass:rrrr > run.out
...
for %%C in ("nistp256" "nistp384") do (
    ...
    echo "Load the %%C ECC signing key under the primary key"
    %TPM_EXE_PATH%load -hp 80000000 -ipr signecc%%Cpriv.bin -ipu signecc%%Cpub.bin -pwdp sto > run.out
    IF !ERRORLEVEL! NEQ 0 (
        exit /B 1
    )
    ...
    for %%H in (%ITERATE_ALGS%) do (
        ...
        %TPM_EXE_PATH%sign -hk 80000001 -halg %%H -salg ecc -scheme ecdsa -if policies/aaa -os sig.bin -pwdk sig > run.out
        IF !ERRORLEVEL! NEQ 0 (
            exit /B 1
        )
        ...
        echo "Verify the signature using PEM - %%H"
        %TPM_EXE_PATH%verifysignature -ipem signecc%%Cpub.pem -halg %%H -if policies/aaa -is sig.bin > run.out
        IF !ERRORLEVEL! NEQ 0 (
            exit /B 1
        )
        ...
        %TPM_EXE_PATH%loadexternal -halg %%H -ecc -ider tmpkeypairecc%%C.der > run.out
        IF !ERRORLEVEL! NEQ 0 (
            exit /B 1
        )
...
for %%H in (%ITERATE_ALGS%) do (
    ...
    %TPM_EXE_PATH%sign -hk 80000001 -halg %%H -salg hmac -scheme hmac -if msg.bin -tk tkt.bin -os sig.bin -pwdk khk > run.out
    IF !ERRORLEVEL! NEQ 0 (
        exit /B 1
    )
...
echo "signapp basic"
%TPM_EXE_PATH%signapp -ic xxx -v > run.out
IF !ERRORLEVEL! NEQ 0 (
    exit /B 1
)

echo "signapp password session"
%TPM_EXE_PATH%signapp -ic xxx -pwsess > run.out
    IF !ERRORLEVEL! NEQ 0 (
        exit /B 1
    )
...
rm tmpkeypaireccnistp256.pem
rm tmpkeypaireccnistp256.der
rm tmpkeypaireccnistp384.pem
rm tmpkeypaireccnistp384.der
rm pssig.bin
rm tmppub.bin
rm tmppub.pem
rm tkt.bin
